...
img_path = 'D:/ocr/Dataset/images'

# Built-in functions for using the pretrained models EAST & CRNN
model = cv2.dnn.readNet('D:/ocr/frozen_east_text_detection.pb')
model_r = cv2.dnn.readNet("D:/ocr/crnn.onnx")

# CRNN set initialization
alphabet_set = "0123456789abcdefghijklmnopqrstuvwxyz.,<>-_*?=()/&%+^'!;:$#{[]}"
blank_char = "-"
char_set = blank_char + alphabet_set
...
def process_image(img):
    ...
    """ IMPORTANT!!!
    A reversal in here might be needed as I lost quite amount of time figuring out what was wrong about the code.
    For some reason getUnconnectedLayers function was giving the layers in the opposite order from the outputs I have
    seen on the internet
    
    IMPORTANT UPDATE!!
    When pipelined error fixed itself somehow no need reverse the output ??
    """
    ...
    """ 2 DIFFERENT METHODS WERE USED TO ELIMINATE REDUNDANT BOUNDING BOXES HOWEVER BOTH FAILED A THIRD METHOD IS USED,
    TURNED INTO A FUNCTION AND GAVE GOOD RESULTS
    #METHOD 1
    final_bb = []
    for c in bb:
        final_bb.append((c[0],c[1],c[2],c[3]))
    print(final_bb)
    final_bb, _ = cv2.groupRectangles(np.array(final_bb).tolist(), 2, 0.5)
    print(final_bb)
    ...
    for rect1 in bb:
        for rect2 in bb:
            if rect1 & rect2 == rect1: # rect1 is completely inside rect2; do nothing.
                continue
            elif (rect1 & rect2).area() > 0: # they intersect; merge them.
                newrect =  rect1 | rect2
    ...
    for (x1, y1, x2, y2) in final_recs:
        ...
        # cv2.waitKey(0)
        blob = cv2.dnn.blobFromImage(segment, scalefactor=1 / 127.5, size=(100, 32), mean=127.5)

        # Pass the image to CRNN
        model_r.setInput(blob)
        scores = model_r.forward()
        # print(scores.shape)
        final_t = best_outcome(scores, char_set)
        # print(final_t)
    ...
    #cv2.waitKey(0)
    # cv2.imwrite(os.path.join('D:/ocr/BB/crnn', name), img_with_bb)
